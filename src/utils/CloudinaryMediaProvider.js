    ...
    return new Promise((res, rej) => {
        form.parse(req, function (err, fields, file) {
            try {
                ...
                if (images[0].name == '') throw new CustomError('Image is required!', 400);
                ...
                return res(Promise.all(
                    images.map(
                        image => new Promise(
                            (resolve, reject) => {
                                cloudinary.uploader.upload(image.filepath, function (error, result) {
...
const validateImage = imageArr => {
    ...
    if (imageArr.length > maxNumberOfImages) return `Exceeded number of files. Expected maximum is ${maxNumberOfImages}.`;

    // Check if they are all images
    const isValidFormat = imageArr.every((image) => validImageFormats.includes(image.originalFilename.split('.')[1]));
    ...
    const isValidSize = imageArr.every(image => image.size <= maxUploadImageSize);
